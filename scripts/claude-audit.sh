#!/usr/bin/env bash
set -euo pipefail
# claude-audit.sh — Analyze Claude Code permission settings
# Usage: bash claude-audit.sh [--global] [--days N] [--verbose]

DAYS=30; VERBOSE=false
while [[ $# -gt 0 ]]; do
  case "$1" in --global) shift;; --days) DAYS="$2"; shift 2;; --verbose) VERBOSE=true; shift;;
    -h|--help) echo "Usage: claude-audit.sh [--global] [--days N] [--verbose]"; exit 0;;
    *) echo "Unknown: $1"; exit 1;; esac
done
