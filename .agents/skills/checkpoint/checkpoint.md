...
- **Never push directly to `<BASE_BRANCH>` or `<PROD_BRANCH>`.** If HEAD is on a protected branch, move to a feature/chore branch first (see Step 4).
...
- If `$b` is `<BASE_BRANCH>` or `<PROD_BRANCH>` → create/switch to a checkpoint branch off current HEAD before staging:
  ```bash
  # Prefer a ticket id if this session has one:
  git checkout -b chore/<PREFIX>-XXX-checkpoint
...
Active ticket: <PREFIX>-XXX — <title>  (state: ...)
...
- If a PR is the right vehicle (branch protection on `<BASE_BRANCH>`/`<PROD_BRANCH>`), offer to open it after the push — don't merge without approval.
